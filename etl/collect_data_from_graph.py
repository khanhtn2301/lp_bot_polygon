...
def get_tick_range_from_hash(w3, tx_hash, tx_type):
    ...
    if tx_type == 'mint':
        ...
        for log in logs_receipt:
            if log['topics'][0].hex() == mint_topic:
                amount = []
                for i in range(1, 4):
                    amount.append(literal_eval('0x' + log['data'].hex()[2 + 64 * i: 66 + 64 * i]))
                return [ctypes.c_long(literal_eval(log['topics'][2].hex())).value, ctypes.c_long(literal_eval(log['topics'][3].hex())).value] + amount
        return 0, 0, 0, 0, 0
    else:
        burn_topic = '0x0c396cd989a39f4459b5fa1aed6a9a8dcdbc45908acfd67e028cd568da98982c'
        for log in logs_receipt:
            if log['topics'][0].hex() == burn_topic:
                amount = []
                for i in range(0, 3):
                    amount.append(literal_eval('0x' + log['data'].hex()[2 + 64 * i: 66 + 64 * i]))
                return [ctypes.c_long(literal_eval(log['topics'][2].hex())).value, ctypes.c_long(literal_eval(log['topics'][3].hex())).value] + amount
